#5) Use a função filter para selecionar apenas as palavras que começam com a letra "a" na lista ["apple", "banana", "avocado", "cherry"].
...
#Utilize a função map para converter todas as palavras da lista ["hello", "world"] em maiúsculas.
...
#7) Crie uma função lambda que receba uma lista de números e retorne a soma deles. Teste com a lista [1, 2, 3, 4].

d = lambda s: sum(s)
print(d([int(o) for o in input().split()]))

#8) Utilize a função filter para remover todos os elementos None da lista [1, None, 2, None, 3, None].
...
#10) Crie uma função lambda que receba uma lista de números e retorne a lista ordenada em ordem decrescente. Teste com a lista [3, 1, 4, 2].
l = [3, 1, 4, 2]
g = lambda f: sorted(f)
